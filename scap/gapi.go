...
type GAPI struct {
	File *string // scap definition to use; nil if undefined
...
func (obj *GAPI) Graph() (*pgraph.Graph, error) {
	if !obj.initialized {
		return nil, fmt.Errorf("scap: GAPI is not initialized")
	}

	config := ParseConfigFromFile(*obj.File) // XXX your own file reader...
	if config == nil {
		return nil, fmt.Errorf("scap: ParseConfigFromFile returned nil")
	}
	...
	if $OS == "Fedora" {
		...
		content := "XXX PARSE FROM SECOND LINE...."
		f0 := &resources.FileRes{
			BaseRes: resources.BaseRes{
				Name:       "README",
				MetaParams: metaparams,
			},
			Path:    "/tmp/mgmt/README", // XXX parse from first line
			Content: &content,
			State:   "present",
		}
		...
		XXX do the same thing but the second part.
	}
